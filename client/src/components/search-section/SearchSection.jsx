  ...
  useEffect(() => {
    const getResultList = async () => {
      const res = await fetch(
        `/api/documents/search?${searchType}=${encodeURIComponent(searchValue)}&offset=${offset - 1}`,
      );
    ...
    const getResultCount = async () => {
      const res = await fetch(`/api/documents/count?${searchType}=${encodeURIComponent(searchValue)}`);
      if (res.status !== 200) {
        history.push('/error');
      }
